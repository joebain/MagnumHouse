...
namespace MagnumHouseLib
{
	public class NetworkHero : NetworkClient, IUpdateable
	{
		Hero hero;
		ObjectHouse house;
		Dictionary<Int32, GangsterVisage> visages = new Dictionary<Int32, GangsterVisage>();
		...
		public void Die() {
			...
			if (hasId) {
				SendMessage(new GoodbyeMessage(id));
			}
			dead = true;
		}
		...
		protected override void HandleMessage ()
		{
			if (GangsterMessage.SIs(readBuffer)) {
				...
				var hm = new HelloMessage(readBuffer);
				id = hm.Id;
				hasId = true;
				Console.WriteLine("got id " + id);
			} else if (GoodbyeMessage.SIs(readBuffer)) {
				var gm = new GoodbyeMessage(readBuffer);
